...
print(RED+"Se estiver usando proxy, desative\nCaso contrario não ira funcionar!!"+RESET)
print(YELLOW+"*"*40+RESET)
print("")
url = input("Digite a URL do site com http/https: ")

##===pode-se incluir mais dorks aqui===##
combinations = [
"view_items.php?id=",
"home.php?cat=",
"item_book.php?CAT=",
...
def search_combinations(base_url, combination, results):
    try:
        full_url = urljoin(base_url, combination)
...
def main():
    print(f"Buscando combinacoes em: {url}\n")
    found_combinations = []
    threads = []

    for combination in combinations:
        thread = threading.Thread(target=search_combinations, args=(url, combination, found_combinations))
        thread.start()
    ...
    print("Resultados:")
    for combination, status_code in found_combinations:
        if status_code == 200:
            print(f"{combination} ===> {status_code}")
